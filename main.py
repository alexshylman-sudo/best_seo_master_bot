...
def patch_db_schema():
    ...
    try:
        ...
        # --- НОВЫЕ ПОЛЯ ДЛЯ БАЗЫ ЗНАНИЙ (СТИЛЬ) ---
...
def generate_and_upload_image(api_url, login, pwd, image_prompt, alt_text, seo_filename, project_style=""):
    ...
    if project_style and len(project_style) > 5:
        # Если есть стиль в базе знаний, добавляем его
        final_prompt = f"Style Reference: {project_style}. Subject: {image_prompt}. Professional photography, 8k, realistic, cinematic lighting."
    else:
        # Дефолтный
...
def menu_handler(message):
    ...
    if txt == "➕ Новый проект":
        ...
        if uid in UPLOAD_STATE: del UPLOAD_STATE[uid] # Сброс состояния загрузки
...
def handle_photo_upload(message):
    ...
    # 1. ОПРЕДЕЛЕНИЕ ТИПА И ЛОГИКА АЛЬБОМОВ
    if message.photo:
        img = message.photo[-1]
        file_id = img.file_id
    ...
    try:
        ...
        # Блокировка строки (FOR UPDATE) чтобы не было гонки при альбоме
        ...
        # Не спамим ошибкой пользователю, если это системная проблема потоков
    finally:
        if conn: 
            cur.close()
            conn.close()
